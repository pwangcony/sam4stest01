...
export const HeaderSlideDrillDown = () => {
  ...
  const handleMouseEnter = (title: string) => {
    ...
    if (activeMenu !== title) setActiveModelId(null); // Reset drill-down
    setActiveMenu(title);
  };
  ...
  const handleMenuClick = (title: string) => {
    if (isLocked && activeMenu === title) {
      ...
      setIsLocked(true);
      if (activeMenu !== title) setActiveModelId(null);
      setActiveMenu(title);
    }
  };
  ...
  return (
    <header className="relative w-full bg-white/80 backdrop-blur-md shadow-sm z-50" onMouseLeave={handleMouseLeave}>
      <div className="max-w-7xl mx-auto px-6">
        <div className="flex justify-between items-center h-16">
          <div className="font-black text-2xl tracking-tighter text-gray-900">SAM4S</div>
          
          <ul className="flex space-x-10 h-16 items-center">
            {menuData.map((menu, idx) => {
              ...
              return (
                ...
                >
                  <span className={`font-bold text-sm transition-colors ${isActive ? 'text-black' : 'text-gray-600 hover:text-black'}`}>
                    {menu.title}
                  </span>
                  {isActive && isLocked && (
                    <div className="absolute bottom-2 left-1/2 -translate-x-1/2 w-1.5 h-1.5 bg-black rounded-full" />
                  )}
                  ...
                  <AnimatePresence>
                    {isActive && (
                      <motion.div 
                        ...
                        className="absolute top-16 -left-20 w-[650px] bg-white shadow-[0_20px_40px_rgba(0,0,0,0.12)] rounded-2xl flex border border-gray-100 overflow-hidden z-50"
                        onMouseEnter={() => {
                          if (leaveTimeoutRef.current) clearTimeout(leaveTimeoutRef.current);
                        }}
                      ...
                      >
                        <div className="w-[35%] bg-[#f8fafc] p-6 flex flex-col items-center justify-center text-center border-r border-gray-100 shrink-0 z-10 relative overflow-hidden">
                           <div className="text-7xl mb-4 relative z-10 hover:scale-110 transition-transform duration-500">{menu.models[0].img}</div>
                        ...
                        <div className="w-[65%] relative h-[360px] overflow-hidden bg-white cursor-default">
                          <div className={`absolute top-0 left-0 w-full h-full flex flex-col transition-transform duration-300 ease-in-out ${activeModelId ? '-translate-x-full' : 'translate-x-0'}`}>
                            ...
                            <div className="px-3 pb-3 flex-1 overflow-y-auto custom-scrollbar">
                              {menu.models.map((model) => (
                                <div key={model.id} onClick={(e) => { e.stopPropagation(); setActiveModelId(model.id); }} className="px-4 py-3.5 mb-1 hover:bg-gray-50 rounded-xl cursor-pointer transition-colors flex justify-between items-center group/item">
                          ...
                          <div className={`absolute top-0 left-0 w-full h-full bg-white flex flex-col transition-transform duration-300 ease-in-out ${activeModelId ? 'translate-x-0' : 'translate-x-full'}`}>
                            <div className="p-4 border-b border-gray-100 flex items-center gap-2 cursor-pointer hover:bg-gray-50 transition-colors shrink-0" onClick={(e) => { e.stopPropagation(); setActiveModelId(null); }}>
